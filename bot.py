...
# 🔥 FORCE JOIN CHANNELS
FORCE_CHANNELS = [
    {"chat_id": "@patelchatting_gc", "link": "https://t.me/patelchatting_gc", "name": "🚩 BACKUP"},
    {"chat_id": "@modxpatel", "link": "https://t.me/modxpatel", "name": "🏴‍☠️ METHOD"}
]
...
def get_user(user_id):
    ...
    if uid not in user_data:
        user_data[uid] = {"points": 10, "daily": None, "ref_by": None, "ref_count": 0, "first_name": "", "username": "", "joined": False}
        save_data()
...
def check_force_join(user_id):
    ...
    # Admin ko force join nahi karna
    if is_admin(user_id):
        return True
    
    # Pehle se joined hai toh check mat karo
    if user_data.get(uid, {}).get("joined", False):
        return True
    ...
    # Agar saare channels mein join hai toh joined mark karo
    user_data[uid]["joined"] = True
    save_data()
    return True, None

def get_force_join_message():
    """Generate force join message"""
    text = """⚠️ <b>Jᴏɪɴ Rᴇǫᴜɪʀᴇᴅ</b> ⚠️

ᴘʟᴇᴀsᴇ ᴊᴏɪɴ ᴛʜᴇsᴇ ᴄʜᴀɴɴᴇʟs ᴛᴏ ᴜsᴇ ᴛʜɪs ʙᴏᴛ:
...
─────────────────
📌 <b>Hᴏᴡ ᴛᴏ Uꜱᴇ?</b>
1️⃣ Cʟɪᴄᴋ ᴏɴ ᴀʟʟ ʟɪɴᴋꜱ ᴀʙᴏᴠᴇ
2️⃣ Jᴏɪɴ ᴛʜᴇ ᴄʜᴀɴɴᴇʟꜱ
3️⃣ Cʟɪᴄᴋ <b>"✅ Cʜᴇᴄᴋ Jᴏɪɴ"</b> ʙᴜᴛᴛᴏɴ
...
def get_force_join_keyboard():
    """Keyboard with join links and check button"""
    keyboard = []
    for channel in FORCE_CHANNELS:
        keyboard.append([{"text": f"📢 {channel['name']}", "url": channel["link"]}])
    keyboard.append([{"text": "✅ Cʜᴇᴄᴋ Jᴏɪɴ", "callback_data": "check_join"}])
...
def format_chat_id_anime(data, query):
    """🔥 Chat ID se number - Clean output"""
            ...
            if resp.status_code == 200:
                ...
                if data.get("ok"):
                    for update in data.get("result", []):
                        ...
                        # ===== CALLBACK QUERY (Check Join Button) =====
                        if "callback_query" in update:
                            ...
                            if callback_data == "check_join":
                                joined, failed_channel = check_force_join(user_id)
                                if joined:
                                    send_msg(chat_id, "✅ <b>Aʟʟ ᴄʜᴀɴɴᴇʟs ᴊᴏɪɴᴇᴅ!</b>\n\n🎉 ɴᴏᴡ ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ᴛʜᴇ ʙᴏᴛ.", parse_mode="HTML")
                                    welcome = get_welcome(callback["message"]["chat"]["first_name"], user_id)
                                    send_msg(chat_id, welcome, reply_markup=get_main_keyboard(), parse_mode="HTML")
                                else:
                                    send_msg(chat_id, f"❌ <b>Jᴏɪɴ Rᴇǫᴜɪʀᴇᴅ</b>\n\nᴘʟᴇᴀsᴇ ᴊᴏɪɴ ᴛʜɪs ᴄʜᴀɴɴᴇʟ:\n{failed_channel['link']}\n\nᴛʜᴇɴ ᴄʟɪᴄᴋ <b>✅ Cʜᴇᴄᴋ Jᴏɪɴ</b>", parse_mode="HTML")
                            continue
                        ...
                        if not is_admin(user_id):
                            joined, failed_channel = check_force_join(user_id)
                            if not joined:
                                send_msg(chat_id, get_force_join_message(), reply_markup=get_force_join_keyboard(), parse_mode="HTML")
                                continue
                        ...
                        if text and text.startswith("/start"):
                            ...
                            if ref_match:
                                ...
                                if str(user_id) != ref_by and not is_banned(ref_by):
                                    ...
                                    uid = str(user_id)
                                    if uid not in user_data:
                                        user_data[uid] = {"points": 10, "daily": None, "ref_by": ref_by, "ref_count": 0, "first_name": first_name, "username": username, "joined": False}
                                    else:
                            ...
                            if text == "⬅️ ᴇxɪᴛ":
                                ...
                                if user_data:
                                    ...
                                    for uid, data in list(user_data.items()):
                                        ...
                                        joined = "✅" if data.get("joined", False) else "❌"
                                        msg_text += f"🆔 {uid}\n   👤 {name} (@{uname})\n   ⭐ {pts} pts | 👥 {ref} refs | {joined} joined\n\n"
